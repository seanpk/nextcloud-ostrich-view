...
Prerequisites: Docker with Compose v2, and git.
...
| `PORT` | no (3000) | Port inside the container. Change the container side of the port mapping in `docker-compose.yml` to match. |
...
`mkdir -p data` matters: the container runs as uid 1000, and if Docker has to
create the bind-mount directory itself it creates it owned by root, leaving the
app unable to write its preview cache. (`./deploy.sh` does this for you.)
...
`localhost:3000` is correct only if `cloudflared` runs on the host (systemd
service, or `network_mode: host`). If `cloudflared` itself runs in a container
on a bridge network, its `localhost` is its own container — point it at
`http://ostrich-view:3000` after putting both containers on the same docker
network, or at `http://host.docker.internal:3000` with the matching
`extra_hosts` entry.

`https://ostrich.example.com` should now show the login page. TLS is
terminated by Cloudflare; the app sets `trustProxy` so rate limiting sees the
real client IP via `CF-Connecting-IP` rather than the tunnel's.
...
1. Refuses to start if `.env` or `config/viewers.json` is missing, and creates
...
4. Polls `http://127.0.0.1:3000/healthz` for up to 30 seconds, then prints
   **PASS** (exit 0) or **FAIL** with the last 50 lines of container output
   (exit 1).
