...
test.describe('A task list', () => {
  ...
  test('a list with nothing finished still says so, calmly', async ({ page }) => {
    await page.goto('/tasks/chores');

    await expect(page.getByRole('heading', { level: 1 })).toHaveText('Chores');
    await expect(page.locator('.section--todo .task__title').first()).toHaveText(
      'Take the bins out'
    );
    await expect(page.locator('.section--done .task__title')).toHaveText('Vacuum the stairs');
  });
});
